...
module Assets =
  type AtomizedContentManager (servicesProvider: IServiceProvider) =
    inherit ContentManager (servicesProvider)

    member this.UnloadAsset (assetName: string) =
      if this.LoadedAssets.ContainsKey assetName
      then
        let asset = this.LoadedAssets.[assetName]
        if asset :? IDisposable
        then (asset :?> IDisposable).Dispose ()
          // this.disposableAssets.Remove (asset) // TODO: update if monogame exposes disposableAssets
      this.LoadedAssets.Remove (assetName)
        
  let mutable content: AtomizedContentManager = Unchecked.defaultof<AtomizedContentManager>
